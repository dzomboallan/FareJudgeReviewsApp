...
public class RestaurantAdapter extends RecyclerView.Adapter<RestaurantAdapter.ViewHolder> {
    ...
    Context context;
    int singledata;
    public ArrayList<RestaurantModel> modelArrayList;
    SQLiteDatabase sqLiteDatabase;

    public RestaurantAdapter(Context context, int singledata, ArrayList<RestaurantModel> modelArrayList, SQLiteDatabase sqLiteDatabase) {
    ...
    public void onBindViewHolder(@NonNull RestaurantAdapter.ViewHolder holder, @SuppressLint("RecyclerView") int position) {
        ...
        final RestaurantModel model = modelArrayList.get(position);
        byte[] image = model.getRestaurantavatar();
        Bitmap bitmap = BitmapFactory.decodeByteArray(image, 0, image.length);
        holder.imageavatar.setImageBitmap(bitmap);
        holder.txtname.setText(model.getRestaurantname());
                    ...
                    public boolean onMenuItemClick(MenuItem menuItem) {
                        switch (menuItem.getItemId()){
                            case R.id.edit_menu:
                                ...
                                bundle.putInt("id", model.getRestaurantid());
                                bundle.putByteArray("avatar", model.getRestaurantavatar());
                                bundle.putString("name", model.getRestaurantname());
                            ...
                            case R.id.delete_menu:
                                ...
                                long recdelete = sqLiteDatabase.delete(RESTAURANTTABLE, "id=" +model.getRestaurantid(), null);
                                if (recdelete != -1 ){
    ...
    public void filterList(ArrayList<RestaurantModel> model) {
