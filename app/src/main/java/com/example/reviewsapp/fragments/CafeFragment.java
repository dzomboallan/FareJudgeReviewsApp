...
public class CafeFragment extends Fragment {
    ...
    CafeAdapter cAdapter;
    FloatingActionButton cafebtn;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        cafebtn = view.findViewById(R.id.cafe_btn);
        ...
        cafebtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent intent = new Intent(getContext(), Cafe.class);
    ...
    private void displayData() {
        ...
        Cursor cursor =sqLiteDatabase.rawQuery("select * from "+CAFETABLE+"", null);
        ArrayList<CafeModel> cmodels = new ArrayList<>();
        while(cursor.moveToNext()){
            int id = cursor.getInt(0);
            byte[] avatar = cursor.getBlob(1);
            String name = cursor.getString(2);

            cmodels.add(new CafeModel(id, avatar, name));
        }
        cursor.close();
        cAdapter = new CafeAdapter(getContext(), R.layout.singledata, cmodels, sqLiteDatabase);
        recyclerView.setAdapter(cAdapter);
    }

    public void filterList(String  text){
        ArrayList<CafeModel> model = new ArrayList<>();
        for (CafeModel Cmodel : cAdapter.modelArrayList){
            if(Cmodel.getCafename().toLowerCase().contains(text.toLowerCase())){
                model.add(Cmodel);
            }
        }
        cAdapter.filterList(model);
    }
}